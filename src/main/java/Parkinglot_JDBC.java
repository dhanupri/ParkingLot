...
public class Parkinglot_JDBC {
    ...
    //comapre the intime with currentTime
    public static List<List<String>> FindCarInTimeInterval(){
        Connection connection=null;
        List<List<String >> arr1=new ArrayList<>();
        try{
            connection=Sql_connection.getCon();
            PreparedStatement ps= connection.prepareStatement("select * from ParkingCars where inTime>= NOW()-INTERVAL 30 MINUTE");
            ResultSet resultSet=ps.executeQuery();
            while (resultSet.next()){
                List<String> arr=new ArrayList<>();
                String plateNo=resultSet.getString("carId");
    ...
    //uc 16 handicap
    public static List<List<String>> FindHandicap(int lotID,String type){
        Connection connection=null;
        List<List<String >> arr1=new ArrayList<>();
        try{
            connection=Sql_connection.getCon();
            PreparedStatement ps= connection.prepareStatement("select * from ParkingCars where lotID=? and drivertype =?");
            ps.setInt(1,lotID);
            ps.setString(2, type);
            ResultSet resultSet=ps.executeQuery();
            while (resultSet.next()){
    ...
    //uc 16 all cars in a parking lot
    public static List<List<String>> FindAllCarInALot(int lotID){
        Connection connection=null;
        List<List<String >> arr1=new ArrayList<>();
        try{
            connection=Sql_connection.getCon();
            PreparedStatement ps= connection.prepareStatement("select * from ParkingCars where lotID=?");
            ps.setInt(1,lotID);
            ResultSet resultSet=ps.executeQuery();
            while (resultSet.next()){
                List<String> arr=new ArrayList<>();
